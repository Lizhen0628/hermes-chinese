...
{/* 本页面由 website/scripts/generate-skill-docs.py 根据该技能的 SKILL.md 自动生成。请编辑源 SKILL.md，而不是本页面。 */}
...
*   `list` / `switch`：管理并在多个已存储的访问令牌之间切换。
*   `whoami`：识别当前登录的账号。
...
*   **Spaces：** 管理交互式应用。包括 `dev-mode` 和 `hot-reload`，可在无需完全重启的情况下运行 Python 文件。

### 存储与自动化
*   **Buckets：** 完整的类 S3 存储桶管理（`create`、`cp`、`mv`、`rm`、`sync`）。
*   **Cache：** 使用 `list`、`prune`（移除游离的修订版本）和 `verify`（校验和检查）管理本地存储。
*   **Webhooks：** 通过管理 Hub webhooks 自动化工作流（`create`、`watch`、`enable`/`disable`）。
*   **Collections：** 将 Hub 项目组织到集合中（`add-item`、`update`、`list`）。
